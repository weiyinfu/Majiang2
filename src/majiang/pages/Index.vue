  ...
  <div class="Index">
    <div class="left" v-if="cli.hand[0]">
      ...
      <div class="actionList" v-if="actionsExceptRelease.length">
        <div v-for="(action,ind) in actionsExceptRelease" :key="ind">
...
import ServerWorker from "../web/server.worker.ts";
import {Ui} from "../web/Ui.ts";
import {FetchResponseMode, ReleaseResponseMode} from "../core/MajiangProtocol.ts";
...
export default {
  ...
  methods: {
    ...
    doAct(action) {
      this.server.postMessage(toRaw(action));
      this.ui.actions = [];
    },
    clickCard(cardInd) {
