  ...
  <div class="DavinceCode" v-if="inited">
    <div class="left">
      ...
      <div v-for="(hand,userId) in cli.hand" :key="userId" class="handCards">
        ...
        <div v-else>
          <card :card="i" v-for="(i,ind) in hand" :key="ind" @click="clickCard(userId,ind)"></card>
        </div>
      </div>
      <div v-if="options" class="optionCards">
        <!--如果有可选项-->
        <card :card="i" v-for="(i,ind) in options" :key="ind" @click="clickWhat(i)"></card>
      </div>
    </div>
    <div class="right" ref="right">
      <div class="messageList">
        <div v-for="(message,ind) in ui.messages" :key="ind">{{ message }}</div>
      </div>
    </div>
  </div>
</template>
...
import Server from "../server.worker.ts";
  ...
  methods: {
    ...
    newGame() {
      ...
      this.ui = new Ui((message) => {
        this.server.postMessage(toRaw(message))
      });
    ...
    doPass() {
      ...
      this.ui.postMessage(toRaw(this.getPassAction()));
      this.options = [];
      this.ui.bestAction = [];
    },
    doCall() {
